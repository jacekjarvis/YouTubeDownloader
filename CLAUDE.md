...
- **The "Select all" header checkbox and the rows sync both ways, guarded against feedback loops.** `MainWindowViewModel.IsSelectAll` pushes its value onto every `VideoItemViewModel.IsSelected` (`OnIsSelectAllChanged`); each row's `IsSelected` change is watched back (`OnVideoPropertyChanged`, wired/unwired per row in `OnVideosCollectionChanged`) and recomputes the header as "all rows selected" (`RecomputeSelectAll`). Both directions set `_syncingSelectAll` first so the reacting side doesn't loop back and reassert what it was just given. `ClearResults` has to unhook rows manually before `Videos.Clear()`, since a Reset `CollectionChanged` carries no `OldItems` for `OnVideosCollectionChanged` to detach from.
